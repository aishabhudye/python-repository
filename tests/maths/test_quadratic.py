...
class MathsProcessorTestCase(unittest.TestCase):
    ...
    def test_2_distinct_real_roots(self):
        ...
        x*x -4*x +3 = 0
        """
        self.assertEqual("2 distinct real roots", self.test_target.solve(1, -4, 3))

    def test_single_real_root(self):
        """
        x*x -2*x +1 = 0
        """
        self.assertEqual("single real root", self.test_target.solve(1, -2, 1))

    def test_no_real_root(self):
        """
        x*x +1 = 0
        """
        self.assertEqual("no real roots", self.test_target.solve(1, 0, 1))
